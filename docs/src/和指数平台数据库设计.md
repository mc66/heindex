...
## 应用运营原始表 app_original_operations

```sql
CREATE TABLE `app_original_operations` (
  ...
  `measure_value` int(11) NOT NULL COMMENT '测量值（1/0）',
  `dimensions` varchar(32) COLLATE utf8_unicode_ci NOT NULL COMMENT '维度',
...
```
## 口碑指数-- 情感指数参数表 app_emotion_parameter（后台）
```sql
