...
public interface UserTrendsMapper {
    ...
    @Select(" SELECT am.total_user,am.penetration_rate FROM app_market am WHERE am.app=#{app} and am.category=#{category} and am.`month`=#{month} ")
    List<Map<String,Object>> queryMonthsDataList(@Param("category") int category,@Param("app") int app,@Param("month") String month);
    @Select(" SELECT am.month,am.penetration_rate FROM app_market am WHERE am.app=#{app} and am.`month` BETWEEN #{month1} and #{month2} ")
    ...
    @Select(" SELECT am.`month`,am.total_user,am.penetration_rate FROM app_market am WHERE am.app=#{app} and am.`month` BETWEEN #{month1} and #{month2} ")
