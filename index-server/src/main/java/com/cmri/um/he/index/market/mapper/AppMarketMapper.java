...
public interface AppMarketMapper {
     ...
     * @return
     */
    @Select("SELECT ai.`name`,FORMAT(am.`monthly_active`,2)AS mau_number,FORMAT(am.`active_next_month_retention_rate`,2) AS keep_rate,FORMAT(am.`monthly_duration_per_person`,2) AS length_time,FORMAT(am.`monthly_dataflow_per_person`,2) AS flow,FORMAT(am.`market_index_score`,2) AS market ,ai.`flag`\n" +
     ...
     * @return
     */
    @Select("SELECT m.id , a.`name` ,FORMAT(m.`monthly_active`,2) AS mau_number , \n" +
            "FORMAT(m.`active_next_month_retention_rate`,2) AS keep_rate ,FORMAT(m.`monthly_duration_per_person`,2) AS length_time ,FORMAT(m.`monthly_dataflow_per_person`,2) AS flow, m.month \n" +
