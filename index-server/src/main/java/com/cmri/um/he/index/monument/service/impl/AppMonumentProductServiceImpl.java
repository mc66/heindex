        ...
        try {
            ...
            if (days<=30){
                for (int i=0;i<days;i++){
                    ...
                    Map<String, Integer> dayCommentAmount = appMonumentProductDao.quaryDayCommentAmount(app, date);
                    freqPositive = dayCommentAmount.get("freqPositive");
                    Integer freqNegativity = dayCommentAmount.get("freqNegativity");
                    Integer freqNeutral = dayCommentAmount.get("freqNeutral");
                    map.put("freqPositive",freqPositive);
                    map.put("freqNegativity",freqNegativity);
                    map.put("freqNeutral",freqNeutral);
                    map.put("petPositive",new BigDecimal((double)(freqPositive/(freqPositive+freqNegativity+freqNeutral)*100)).setScale(2,BigDecimal.ROUND_HALF_UP)+"%");
                    map.put("petNegativity",new BigDecimal((double)(freqNegativity/(freqPositive+freqNegativity+freqNeutral)*100)).setScale(2,BigDecimal.ROUND_HALF_UP)+"%");
                    map.put("petNeutral",new BigDecimal((double)(freqNeutral/(freqPositive+freqNegativity+freqNeutral)*100)).setScale(2,BigDecimal.ROUND_HALF_UP)+"%");
                    map.put("month",date);
                    list.add(map);
                }
            }else {
                ...
                for (int j=0;j<count;j++){
                    ...
                    long freqPositive = (long)commentAmount.get("freqPositive");
                    long freqNegativity = (long)commentAmount.get("freqNegativity");
                    long freqNeutral = (long)commentAmount.get("freqNeutral");
                    map.put("freqPositive",freqPositive);
                    map.put("freqNegativity",freqNegativity);
                    map.put("freqNeutral",freqNeutral);
                    map.put("petPositive",new BigDecimal((double)(freqPositive/(freqPositive+freqNegativity+freqNeutral)*100)).setScale(2,BigDecimal.ROUND_HALF_UP)+"%");
